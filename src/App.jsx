  ...
  return (
    <div className='cal'>
      ...
      <div className='grid'>
        ...
        <button value="/" onClick={(e) => setVal(val + e.target.value)}>/</button>
        <button value="*" onClick={(e) => setVal(val + e.target.value)}>*</button>
        <button value="%" onClick={() => setVal((parseFloat(val) / 100).toString())}>%</button>
        <button className="span-2" value="C" onClick={() => backspace()}>C</button>
        <button className="span-2" value="=" onClick={() => calculation()}>=</button>
      </div>
    </div>
  );
}
